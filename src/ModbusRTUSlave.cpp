...
ModbusRTUSlave::ModbusRTUSlave(HardwareSerial &serial, uint8_t dePin, void (*OnPollData)(String value))
{
...
void ModbusRTUSlave::poll()
{
  if (_serial->available())
  {
    if (_readRequest())
    {
      if (_buf[0] == _id || _buf[0] == 0)
      {
        if (_OnPollData)
          _OnPollData("been Polled");
      }
      switch (_buf[1])
      {
      case 1:
...
void ModbusRTUSlave::_calculateTimeouts(unsigned long baud, uint32_t config)
{
  ...
  if (config == SERIAL_8E2 || config == SERIAL_8O2)
    bitsPerChar = 12;
  else if (config == SERIAL_8N2 || config == SERIAL_8E1 || config == SERIAL_8O1)
    bitsPerChar = 11;
  else
    bitsPerChar = 10;
  if (baud <= 19200)
